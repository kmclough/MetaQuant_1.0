...
public:
  ...
  /**
   * A member function that returns the 5' bias value at a given position
   * in a given target based on the current bias parameters.
   * @param targ the target for which to calculate the bias.
   * @param pos the target for which to calculate the bias.
   * @return The 5' bias (logged).
   */
  double get_target_start_bias(const Target& targ, size_t pos) const;
  /**
   * A member function that returns the 3' bias value at a given position
   * in a given target based on the current bias parameters.
   * @param targ the target for which to calculate the bias.
   * @param pos the target for which to calculate the bias.
   * @return The 3' bias (logged).
   */
  double get_target_end_bias(const Target& targ, size_t pos) const;
  /**
   * A member function that computes the 5' and 3' bias values at each position
   * in a given target based on the current bias parameters, and then computes
   * the product of their averages. Uses the cached values in start_bias and end_bias
   * when they exist for a particular position, otherwise calculates them on the fly.
   * @param start_bias a map from fragment 5' start positions to the logged bias for 
   *        those positions in the target.
   * @param end_bias a map from fragment 3' end positions to the logged bias for 
   *        those positions in the target.
   * @param targ the target for which to calculate the average bias.
   * @return The product of the average 5' and 3' bias (logged).
   */
  double get_average_target_bias(std::map<const size_t, float>& start_bias,
                                 std::map<const size_t, float>& end_bias, 
                                 const Target& targ) const;
  /**
   * A member function that updates the cached 5' and 3' bias values for each position
   * in a given target that is aligned to the start or end of a fragment, based on 
   * the current bias parameters.
   * @param start_bias a map from fragment 5' start positions to the logged bias for 
   *        those positions in the target.
   * @param end_bias a map from fragment 3' end positions to the logged bias for 
   *        those positions in the target.
   * @param targ the target for which to update the biases.
   */
  void update_target_bias(std::map<const size_t, float>& start_bias,
                         std::map<const size_t, float>& end_bias,
                         const Target& targ) const;
  /**
