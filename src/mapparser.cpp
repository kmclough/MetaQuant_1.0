...
MapParser::MapParser(Library* lib, bool write_active)
  ...
  bool is_sam = false;
  if (in_file.size() == 0) {
    logger.severe("No alignment file specified. Streaming input not supported in MetaQuant\n");
  } else {
...
size_t MapParser::count_fragments(size_t stop_at) {
  if (stop_at) {
    return stop_at;
  }
  logger.info("Counting fragments...");
  ...
  while (!stop_at || n < stop_at) {
    ...
    for (size_t i = 0; i < frag->num_hits(); i++) {
      ...
      add_hit_target(hit->target_id());
    }
    delete frag;
    if (n % 100000 == 0) {
      TargIDSet& hit_targets = hit_target_ids();
      logger.info("...Counted %d fragments so far with hits to %d targets.", n, 
         hit_targets.size());
    }
    n++;
  }
  TargIDSet& hit_targets = hit_target_ids();
  logger.info("Counted %d fragments in all with hits to %d targets.", n, hit_targets.size());
  return n;
}
  ...
  while (p && i <= 9) {
    switch(i++) {
      case 0: {
        ...
        if (boost::algorithm::ends_with(r.name, "\1") ||
            boost::algorithm::ends_with(r.name, "\2") ||
            boost::algorithm::ends_with(r.name, ".1") ||
            boost::algorithm::ends_with(r.name, ".2") ) {
          r.name = r.name.substr(0, r.name.size()-2);
        }
        //logger.info("Got read %s", r.name.c_str());
        break;
      }
      case 1: {
        sam_flag = atoi(p);
        ...
        if (paired && (direction == F || direction == R)) {
          //logger.info("Read %s: wrong direction for paired", r.name.c_str());
          goto stop;
        }
        ...
        if (((direction == RF || direction == R) && left_first) ||
            ((direction == FR || direction == F) && !left_first)) {
                    //logger.info("Read %s: paired reads in wrong order", r.name.c_str());
  		    goto stop;
        }
        break;
      }
      ...
      case 7: {
        ...
        if (paired && ((r.reversed && r.left < (size_t)r.mate_l) ||
          ...
          //logger.info("Read %s: wrong length for mate", r.name.c_str());
          goto stop;
        }
        break;
      }
